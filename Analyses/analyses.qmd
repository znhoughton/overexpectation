...
uo_data = read_csv('../Data/uo-data.csv')
ucd_data = read_csv('../Data/ucd-data.csv')
...
data_filtered = data %>%
  select(participant, date, mouse_3.clicked_name, mouse_3.x, mouse_3.y, mouse_3.corr, mouse.clicked_name, mouse_4.clicked_name, mouse_phase, Picture1, Picture2, TestPicture1, TestPicture2, Sound, Sound1, sizeScalar) %>%
  ...
  select(-Sound1, -TestPicture1, -TestPicture2) %>%
...
We should filter out data before 2/4/2026

(between 1/15 and 2/4 are the first round of results, which did not have the size differences in stage 3)

```{r}
data_filtered$date = as.Date(data_filtered$date)

cutoff = ymd("2026-02-04")

data_filtered = data_filtered %>%
  filter(date > cutoff)

data_filtered$sizeScalar = factor(data_filtered$sizeScalar)
```
...
m1_f0 = brm(
  resp ~ sizeScalar * Sound * mouse_phase + (1|participant),
...
m2_f0 = brm(
  resp ~ Sound * sizeScalar + (1 + Sound * sizeScalar |participant),
...
plot_accuracy_props_grid <- function(df, choices, stages = NULL) {
  df %>%
    { if (!is.null(stages))
        filter(., mouse_phase %in% stages)
      else .
    } %>%
    mutate(mouse_phase = droplevels(mouse_phase)) %>%
    filter(pitch_cat %in% choices) %>%
    group_by(mouse_phase, condition, sizeScalar, pitch_cat) %>%
    summarise(
      n_trials  = n(),
    ...
    complete(
      ...
      sizeScalar,
      pitch_cat,
      fill = list(n_trials = 0, n_correct = 0)
    ) %>%
    mutate(
    ...
    facet_grid(
      ...
      labeller = labeller(
        mouse_phase = c("1"="Stage 1","2"="Stage 1","3"="Stage 2"),
